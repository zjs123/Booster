...
def prepare_dataset_rels(path, name, fact_count):
    ...
    for f in files:
        ...
        for line in to_read.readlines():
            ...
            if start_idx < 0:
                examples.append([int(triple[0]), int(triple[1]), int(triple[2]), end_idx])
            else:
    ...
    for f in pickle_files:
        for lhs, rel, rhs, t in f:
            to_skip['lhs'][(rhs, rel + len(relations), t)].add(lhs)  # reciprocals
            to_skip['rhs'][(lhs, rel, t)].add(rhs)
